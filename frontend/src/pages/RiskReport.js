...
export default function RiskReport() {
  ...
  const buildForecast = () => {
    ...
    for (let d = 0; d <= 30; d += 5) {
      ...
      points.push({
        ...
        exp: Math.round(
          (report.trends?.expenses?.current || 0) * (0.9 + 0.35 * t + (isCritical ? 0.08 : 0))
        ),
      });
    }
    return points;
  ...
  const topDomainLabel = topDomain
    ? lang === 'fr'
      ? DOMAIN_FR[topDomain.key]
      : DOMAIN_EN[topDomain.key]
    : '';
  ...
  return (
    <div>
      ...
      <section className="mb-7 flex items-start justify-between gap-6 flex-wrap">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-3 mb-3 flex-wrap">
            <span
              className={`editorial-pill-critical`}
              style={{
            ...
            <span className="text-[12px] text-[#6b7280] italic">
              {lang === 'fr'
                ? "Analyse prédictive générée par l'IA le "
                : 'AI-generated predictive analysis on '}
          ...
          <h1
            className="font-editorial text-[40px] md:text-[44px] font-black leading-[1.05] text-[#0b1f33]"
        ...
        <div
          className="editorial-card px-5 py-4 flex items-center gap-3 shrink-0"
        ...
        >
          <div>
            ...
            <div
              className="font-editorial text-[36px] font-black leading-none"
              style={{
            ...
            >
              {score}
              <span className="text-[16px] text-[#6b7280] font-normal">/100</span>
            </div>
          </div>
          <div
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 mb-7">
        ...
        <div className="lg:col-span-8 editorial-card p-7">
          <div className="flex items-start justify-between mb-4 flex-wrap gap-2">
            <div>
              <h3
                className="font-editorial text-[20px] font-bold text-[#0b1f33]"
                style={{ fontFamily: 'Playfair Display, Georgia, serif' }}
              >
                {lang === 'fr'
              ...
              <p className="text-[12px] text-[#6b7280] mt-1">
                {lang === 'fr'
                  ? 'Basé sur le cycle de facturation actuel et les projections de dépenses'
            ...
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#eef3f8] text-[#002b4c] text-[11px] font-semibold">
              ...
              {lang === 'fr' ? 'Précision du modèle : ' : 'Model precision: '}
          ...
          <div style={{ height: 270 }}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={forecast} margin={{ top: 10, right: 15, left: 0, bottom: 0 }}>
                ...
                />
                <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: '#8b8672' }} />
                <Tooltip
                  contentStyle={{ borderRadius: 8, border: '1px solid #e5ddce', fontSize: 12 }}
                />
                <Legend
                  wrapperStyle={{
                    fontSize: 11,
                ...
                <Line
                  type="monotone"
                  dataKey="rev"
                  name={lang === 'fr' ? 'Revenus prévus' : 'Expected revenue'}
                  stroke="#002b4c"
                  strokeWidth={2.5}
                  dot={false}
                />
                <Line
                  type="monotone"
                  dataKey="exp"
                  name={lang === 'fr' ? 'Dépenses prévues' : 'Expected expenses'}
                  stroke="#c8102e"
                  strokeWidth={2.5}
                  dot={false}
                />
        ...
        {/* Narrative analysis */}
        <div
          className="lg:col-span-4 editorial-card p-6 relative"
          style={{
            borderLeft: `4px solid ${isCritical ? '#c8102e' : '#0d7a4a'}`,
        ...
        >
          ...
          >
            <span className="material-symbols-outlined text-[20px]" style={{ color: '#002b4c' }}>
              auto_awesome
            </span>
          </div>
          <h3
            className="font-editorial text-[18px] font-bold text-[#0b1f33] mb-2"
            style={{ fontFamily: 'Playfair Display, Georgia, serif' }}
          >
          ...
          <p className="text-[13px] text-[#4a5568] leading-relaxed">
            {lang === 'fr' ? (
              isCritical ? (
                <>
                  Le système a détecté une exposition significative sur le domaine{' '}
                  <strong>{topDomainLabel}</strong>, avec un score de{' '}
                  <strong style={{ color: '#c8102e' }}>{topDomain?.score}/100</strong>. La tendance
                  actuelle indique un{' '}
                  <strong style={{ color: '#c8102e' }}>déficit potentiel</strong> si aucune action
                  corrective n'est entreprise dans les prochaines semaines.
                </>
              ) : (
                <>
                  Le système détecte une activité économique équilibrée. Le domaine dominant{' '}
              ...
              <>
                The system has detected a significant exposure on the{' '}
                <strong>{topDomainLabel}</strong> domain, with a score of{' '}
                <strong style={{ color: '#c8102e' }}>{topDomain?.score}/100</strong>. The current
                trend points to a <strong style={{ color: '#c8102e' }}>potential deficit</strong> if
                no corrective action is taken in the coming weeks.
              </>
            ) : (
              <>
                The system detects balanced economic activity. Dominant domain{' '}
                <strong>{topDomainLabel}</strong>
          ...
          <div
            ...
            style={{ borderTop: '1px solid #e5ddce' }}
          >
            <span className="small-caps text-[#6b7280]">
              {lang === 'fr' ? 'Impact estimé' : 'Estimated impact'}
            </span>
            <span
              className="font-editorial text-[22px] font-black"
              style={{
                fontFamily: 'Playfair Display, Georgia, serif',
            ...
            >
              {impact >= 0 ? '+' : ''}
              {Math.round(impact).toLocaleString('fr-FR')} TND
            </span>
          </div>
        </div>
      </div>
      ...
      <section className="mb-10">
        <h3
          className="font-editorial text-[24px] font-extrabold text-[#0b1f33] mb-5"
          style={{ fontFamily: 'Playfair Display, Georgia, serif' }}
        >
