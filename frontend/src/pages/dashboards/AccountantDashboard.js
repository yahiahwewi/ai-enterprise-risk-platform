...
export default function AccountantDashboard() {
  ...
  const [txForm, setTxForm] = useState({ type: 'income', amount: '', category: '', description: '' });
  ...
  useEffect(() => {
    Promise.all([api.get('/transactions'), api.get('/invoices')])
      .then(([tx, inv]) => { setTransactions(tx.data); setInvoices(inv.data); })
  ...
  const updateInvoiceStatus = async (id, status) => {
    ...
    } catch { addToast('error', t('toast.error'), 'Failed to update'); }
  };

  if (loading) return (<div><section className="mb-10"><h2 className="text-3xl font-extrabold font-headline tracking-tight">{t('accountant.title')}</h2></section><SkeletonKPIGrid count={4} /><SkeletonTable /></div>);
  ...
  return (
    <div>
      <section className="mb-10">
        <h2 className="text-3xl font-extrabold font-headline tracking-tight text-on-surface dark:text-slate-100">{t('accountant.title')}</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
        <div className="bg-surface-container-lowest dark:bg-slate-800 rounded-xl p-6">
          <h3 className="text-base font-bold font-headline text-on-surface dark:text-slate-100 mb-4">{t('accountant.addTransaction')}</h3>
          <form onSubmit={addTransaction} className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div><label className="block text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mb-1">{t('accountant.type')}</label><select value={txForm.type} onChange={(e) => setTxForm({ ...txForm, type: e.target.value })} className="w-full bg-surface-container-low dark:bg-slate-700 border-none rounded-lg py-2 px-3 text-sm text-on-surface dark:text-slate-200"><option value="income">{t('accountant.income')}</option><option value="expense">{t('accountant.expense')}</option></select></div>
        ...
        <div className="bg-surface-container-lowest dark:bg-slate-800 rounded-xl p-6">
          ...
          <form onSubmit={addInvoice} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className="block text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mb-1">{t('accountant.dueDate')}</label><input type="date" value={invForm.dueDate} onChange={(e) => setInvForm({ ...invForm, dueDate: e.target.value })} className="w-full bg-surface-container-low dark:bg-slate-700 border-none rounded-lg py-2 px-3 text-sm text-on-surface dark:text-slate-200" required /></div>
              <div><label className="block text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mb-1">{t('common.status')}</label><select value={invForm.status} onChange={(e) => setInvForm({ ...invForm, status: e.target.value })} className="w-full bg-surface-container-low dark:bg-slate-700 border-none rounded-lg py-2 px-3 text-sm text-on-surface dark:text-slate-200"><option value="pending">Pending</option><option value="paid">Paid</option><option value="late">Late</option></select></div>
            </div>
            <button type="submit" className="executive-gradient text-white text-xs font-bold px-4 py-2 rounded-lg hover:opacity-90 transition-opacity">{t('accountant.addInvoice')}</button>
          </form>
        </div>
      </div>
      ...
      <div className="bg-surface-container-lowest dark:bg-slate-800 rounded-xl p-6 mb-8">
        <h3 className="text-base font-bold font-headline text-on-surface dark:text-slate-100 mb-4">{t('accountant.recentTransactions')}</h3>
        {transactions.length === 0 ? <EmptyState icon="receipt_long" title={t('accountant.noTransactions')} message={t('accountant.noTransactionsMsg')} /> : (
      ...
      <div className="bg-surface-container-lowest dark:bg-slate-800 rounded-xl p-6">
        <h3 className="text-base font-bold font-headline text-on-surface dark:text-slate-100 mb-4">{t('accountant.invoiceList')}</h3>
        {invoices.length === 0 ? <EmptyState icon="description" title={t('accountant.noInvoices')} message={t('accountant.noInvoicesMsg')} /> : (
          <div className="overflow-x-auto"><table className="w-full"><thead><tr className="text-left">
