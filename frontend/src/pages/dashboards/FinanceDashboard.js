...
export default function FinanceDashboard() {
  ...
  const addLoan = async (e) => {
    ...
    try {
      ...
      setLoans([data, ...loans]);
      setLoanForm({ amount: '', interestRate: '', duration: '', monthlyPayment: '' });
  ...
  const addAsset = async (e) => {
    ...
    try {
      ...
      setAssetForm({ name: '', value: '', depreciationRate: '' });
    ...
    return (
      <div>
        <section className="mb-10">
          <h2 className="text-3xl font-extrabold font-headline">{t('financeD.title')}</h2>
        </section>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
        ...
        />
        <KPICard
          label={t('financeD.monthlyPayments')}
        ...
        />
        <KPICard
          label={t('financeD.debtToAsset')}
          value={ratio}
          icon="analytics"
          iconColor="blue"
          progress={Math.min(parseFloat(ratio) * 50, 100) || 0}
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
        <div className="bg-surface-container-lowest dark:bg-slate-800 rounded-xl p-6">
          ...
          <form onSubmit={addLoan} className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className={labelCls}>{t('financeD.interestRate')}</label>
                <input
                  type="number"
                  step="0.01"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className={labelCls}>{t('financeD.duration')}</label>
                <input
                  type="number"
              ...
              <div>
                <label className={labelCls}>{t('financeD.monthlyPayment')}</label>
                <input
                  type="number"
            ...
            <button
              type="submit"
              className="executive-gradient text-white text-xs font-bold px-4 py-2 rounded-lg hover:opacity-90"
            >
              {t('financeD.addLoan')}
            </button>
          </form>
        ...
        <div className="bg-surface-container-lowest dark:bg-slate-800 rounded-xl p-6">
          ...
          <form onSubmit={addAsset} className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <ComboInput
                  ...
                  onChange={(val) => setAssetForm({ ...assetForm, name: val })}
                  options={[...new Set(assets.map((a) => a.name).filter(Boolean))]}
                  placeholder={t('financeD.selectAsset')}
                  label="asset"
                  required
                />
              </div>
              <div>
                <label className={labelCls}>{t('financeD.value')}</label>
                <input
                  type="number"
            ...
            <div>
              <label className={labelCls}>{t('financeD.depreciationRate')}</label>
              <input
                type="number"
                step="0.1"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-surface-container-lowest dark:bg-slate-800 rounded-xl p-6">
          ...
          {loans.length === 0 ? (
            <EmptyState
              ...
              title={t('financeD.noLoans')}
              message={t('financeD.noLoansMsg')}
            />
          ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="text-left">
                    ...
                    <th className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest pb-3">
                      {t('financeD.interestRate')}
                    </th>
                    <th className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest pb-3">
                      {t('financeD.duration')}
                    </th>
                    <th className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest pb-3">
                      {t('financeD.monthlyPayment')}
                    </th>
                  </tr>
                </thead>
                  ...
                  {loans.map((l) => (
                    ...
                    >
                      <td className="py-3 text-sm font-medium text-on-surface dark:text-slate-200">
                        {l.amount.toLocaleString()} TND
                      </td>
                      <td className="py-3 text-sm text-on-surface-variant">{l.interestRate}%</td>
                      <td className="py-3 text-sm text-on-surface-variant">{l.duration}mo</td>
                      <td className="py-3 text-sm font-bold text-on-surface dark:text-slate-200">
                        {l.monthlyPayment.toLocaleString()} TND
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="bg-surface-container-lowest dark:bg-slate-800 rounded-xl p-6">
          ...
          {assets.length === 0 ? (
            <EmptyState
              icon="inventory_2"
              title={t('financeD.noAssets')}
              message={t('financeD.noAssetsMsg')}
            />
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full">
