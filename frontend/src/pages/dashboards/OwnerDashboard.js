    ...
    try {
      ...
      addToast('success',
        scenario === 'good'
          ...
          : (lang === 'fr' ? '⚠ Mauvais scénario chargé' : '⚠ Bad scenario loaded'),
        res.data.message);
      await onRefresh();
    } catch (err) {
    ...
    <div className="mb-6">
      <button onClick={() => setOpen((o) => !o)}
        ...
        <span className="material-symbols-outlined text-[14px]"
              style={{ transform: open ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.2s' }}>expand_more</span>
      </button>
      {open && (
        <div className="mt-3 flex gap-2">
          <button onClick={() => seed('good')} disabled={!!seeding}
          ...
          <button onClick={() => seed('bad')} disabled={!!seeding}
                  className="editorial-btn-ghost text-xs disabled:opacity-50"
...
export default function OwnerDashboard() {
  ...
  if (!report) return <div className="editorial-card p-8 text-center text-[#6b7280]">{t('common.noData')}</div>;
  ...
  const decision = (() => {
    if (score >= 75) return {
      banner:  lang === 'fr' ? 'DÉCISION : RISQUE CRITIQUE — ACTION IMMÉDIATE' : 'DECISION: CRITICAL RISK — IMMEDIATE ACTION',
      sub:     lang === 'fr' ? 'Une intervention stratégique immédiate est requise pour stabiliser l\'entreprise.'
                              : 'An immediate strategic intervention is required to stabilise the company.',
      cta:     lang === 'fr' ? 'Alerte Critique' : 'Critical Alert',
    };
    if (score >= 50) return {
      banner:  lang === 'fr' ? 'DÉCISION : RISQUE ÉLEVÉ — ACTION REQUISE' : 'DECISION: HIGH RISK — ACTION REQUIRED',
      sub:     lang === 'fr' ? 'Une intervention stratégique immédiate est recommandée pour stabiliser les flux de trésorerie.'
                              : 'An immediate strategic intervention is recommended to stabilise cash flows.',
      cta:     lang === 'fr' ? 'Alerte Critique' : 'Critical Alert',
    };
    if (score >= 25) return {
      banner:  lang === 'fr' ? 'DÉCISION : RISQUE MODÉRÉ — SURVEILLANCE' : 'DECISION: MODERATE RISK — MONITOR',
      sub:     lang === 'fr' ? 'Des signaux avant-coureurs sont détectés, un suivi rapproché est conseillé.'
    ...
    return {
      ...
      sub:    lang === 'fr' ? 'Aucun risque majeur détecté ce mois-ci. Poursuivre la stratégie actuelle.'
  ...
  return (
    <div>
      ...
      {/* ─── DECISION BANNER ─── */}
      <div className={`mb-8 ${isAlert ? 'editorial-alert-banner' : 'editorial-card'}`}
           style={!isAlert ? { borderLeft: '6px solid #0d7a4a', padding: '22px 28px' } : undefined}>
        <div className="flex items-start justify-between gap-4 flex-wrap relative z-10">
          <div className="flex-1 min-w-0">
            <h2 className={`font-editorial text-[26px] md:text-[32px] font-black leading-tight ${isAlert ? 'text-white' : 'text-[#0b1f33]'}`}
          ...
          <Link to="/alerts"
                className={`shrink-0 px-5 py-2.5 rounded-lg text-sm font-semibold transition-all
                  ${isAlert ? 'bg-white/15 hover:bg-white/25 text-white border border-white/30'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-8">
        {/* Score card */}
        <div className="lg:col-span-4 editorial-card p-8 flex flex-col items-center justify-center text-center">
          <span className="small-caps text-[#6b7280] mb-5">{lang === 'fr' ? 'Global Risk Score' : 'Global Risk Score'}</span>
          <ScoreDonut score={score} level={level} />
        </div>

        {/* Narrative */}
        <div className="lg:col-span-8 editorial-card p-8 relative overflow-hidden"
             style={{ borderLeft: isAlert ? '4px solid #c8102e' : '4px solid #002b4c' }}>
          <div className="flex items-start gap-4">
            <div className="mt-1 shrink-0">
              <span className={`material-symbols-outlined text-[28px] ${isAlert ? 'text-[#c8102e]' : 'text-[#002b4c]'}`}>
                {isAlert ? 'trending_down' : 'trending_flat'}
              </span>
            </div>
            <div className="min-w-0">
              <h3 className="font-editorial text-[22px] md:text-[26px] font-bold leading-snug text-[#0b1f33]"
                  style={{ fontFamily: 'Playfair Display, Georgia, serif' }}>
                {lang === 'fr'
                  ? (isAlert
                      ? `Votre entreprise est à risque ${level === 'critical' ? 'critique' : 'élevé'} en raison d'une exposition sur le domaine ${topCauses[0]?.fr?.toLowerCase() || 'trésorerie'}.`
                      : 'Votre entreprise maintient un niveau de risque maîtrisé ce mois-ci.')
                  : (isAlert
                      ? `Your company is at ${level === 'critical' ? 'critical' : 'high'} risk, driven by ${topCauses[0]?.en?.toLowerCase() || 'cash flow'}.`
                      : 'Your company is in a stable state this month.')}
              </h3>
              <div className="mt-4 flex items-center gap-2">
                <span className="editorial-badge-ai">AI</span>
                <p className="text-xs text-[#6b7280] italic">
                  {lang === 'fr' ? 'Analyse prédictive basée sur les cycles de facturation actuels.'
                                  : 'Predictive analysis based on current billing cycles.'}
                </p>
              </div>
            </div>
          </div>
      ...
      <section className="mb-10">
        <div className="flex items-end justify-between mb-5">
          <h3 className="font-editorial text-[24px] font-extrabold text-[#0b1f33]"
              style={{ fontFamily: 'Playfair Display, Georgia, serif' }}>
          ...
          <div className="flex items-center gap-1 small-caps text-[#8b8672]">
            ...
            <div className="ml-2 w-7 h-7 rounded-full flex items-center justify-center"
          ...
          {recs.map((text, i) => {
            ...
            const defaults = [
              { title: lang === 'fr' ? 'Contacter les clients principaux' : 'Contact key clients',
                body:  lang === 'fr' ? 'Envoyer des rappels automatisés pour les factures impayées.' : 'Send automated reminders for unpaid invoices.',
                cta:   lang === 'fr' ? 'Lancer les rappels' : 'Trigger reminders',
                icon:  'group',
                link:  '/invoices' },
              { title: lang === 'fr' ? 'Geler les dépenses non essentielles' : 'Freeze non-essential spending',
                body:  lang === 'fr' ? 'Reporter les achats d\'équipement non critiques pour le trimestre.' : 'Postpone non-critical equipment purchases this quarter.',
                cta:   lang === 'fr' ? 'Appliquer le gel' : 'Apply freeze',
                icon:  'block',
                link:  '/transactions' },
              { title: lang === 'fr' ? 'Reporter les nouvelles embauches' : 'Delay new hires',
                body:  lang === 'fr' ? 'Suspendre le recrutement ouvert pour préserver les réserves de trésorerie.' : 'Suspend open recruitment to preserve cash reserves.',
...
function ScoreDonut({ score, level }) {
  const color =
    level === 'critical' ? '#c8102e' :
    level === 'high'     ? '#e67e22' :
  ...
  return (
    <div className="relative w-52 h-52 flex items-center justify-center">
      <svg className="w-full h-full transform -rotate-90" viewBox="0 0 192 192">
        <circle cx="96" cy="96" r="84" fill="transparent" stroke="#efe8dc" strokeWidth="14" />
        <circle cx="96" cy="96" r="84" fill="transparent" stroke={color} strokeWidth="14"
                strokeDasharray={circumference} strokeDashoffset={dashOffset}
                strokeLinecap="round" className="transition-all duration-1000" />
      </svg>
      <div className="absolute flex flex-col items-center">
        <span className="font-editorial text-[56px] font-black leading-none tracking-tight text-[#0b1f33]"
              style={{ fontFamily: 'Playfair Display, Georgia, serif' }}>{score}</span>
        <span className="small-caps mt-2" style={{ color: '#6b7280' }}>sur 100</span>
      </div>
    </div>
  );
}
