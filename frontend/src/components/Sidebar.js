...
function useNavItems(t, lang) {
  return {
    admin: [
      ...
      {
        to: '/approvals',
        label: lang === 'fr' ? 'Approbations' : 'Approvals',
      ...
      {
        to: '/email-config',
        label: lang === 'fr' ? 'Notifications email' : 'Email notifications',
    ...
    owner: [
      ...
      {
        to: '/executive',
        label: lang === 'fr' ? 'Vue Exécutive' : 'Executive View',
        icon: 'monitoring',
      },
      ...
      {
        to: '/alerts',
        label: lang === 'fr' ? 'Alertes critiques' : 'Critical alerts',
        icon: 'crisis_alert',
      },
      {
        to: '/investigations',
      ...
      {
        to: '/extract-invoice',
        label: lang === 'fr' ? 'Import Facture IA' : 'AI Invoice Import',
    ...
    auditor: [
      ...
      {
        to: '/audit',
        label: lang === 'fr' ? 'Tableau de bord audit' : 'Audit dashboard',
        icon: 'fact_check',
      },
      {
        to: '/investigations',
      ...
      {
        to: '/reports',
        label: lang === 'fr' ? 'Rapports certifiés' : 'Certified reports',
        icon: 'picture_as_pdf',
      },
      { to: '/activity', label: t('nav.activityLog'), icon: 'history' },
  ...
  return (
    ...
    >
      ...
      <div className="px-6 mb-7">
        <div
          className="text-2xl font-headline font-extrabold text-editorial-ink leading-tight"
      ...
      <nav className="flex-1 space-y-0.5 overflow-y-auto px-1">
        {items.map((item, i) =>
          item.section ? (
            <div key={`s-${i}`} className="px-5 pt-5 pb-2 small-caps" style={{ color: '#8b8672' }}>
              {item.section}
            </div>
          ) : (
            <NavLink
      ...
      <div className="mt-auto px-4 space-y-3 pt-4" style={{ borderTop: '1px solid #e5ddce' }}>
        ...
        <div className="flex items-center justify-between px-1">
          <div className="flex gap-1">
            <button
              ...
              style={{
                background: lang === 'fr' ? '#002b4c' : 'transparent',
                color: lang === 'fr' ? '#ffffff' : '#6b7280',
              }}
            >
              FR
            </button>
            <button
              ...
              className={`text-[10px] font-bold py-1 px-2 rounded-md transition-colors small-caps`}
              style={{
                background: lang === 'en' ? '#002b4c' : 'transparent',
        ...
        <div className="flex items-center gap-3 p-3 rounded-lg" style={{ background: '#f0eadb' }}>
          <div
          ...
          <div className="overflow-hidden">
            <p className="text-sm font-bold truncate" style={{ color: '#0b1f33' }}>
              {user?.name}
            </p>
            <p className="small-caps" style={{ color: '#8b8672' }}>
              {user?.role}
            </p>
          </div>
        </div>
